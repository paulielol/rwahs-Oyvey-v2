...
import me.alpha432.oyvey.features.modules.client.Media;
    ...
    public void renderStringAtPosHook(FontRenderer renderer, String text, boolean shadow) {
        if (Media.getInstance().isOn()) {
            ...
            if (NickHider.getInstance().isOn() && NickHider.getInstance().changeOwn.getValue().booleanValue()) {
                this.renderStringAtPos(text.replace(NickHider.getInstance().enemyName.getValue(), NickHider.getInstance().enemyNewName.getValue().toString()), shadow);
            } else {
