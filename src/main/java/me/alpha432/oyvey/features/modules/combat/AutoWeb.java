...
public class AutoWeb
    ...
    private final Setting<Boolean> silentSwitch = this.register(new Setting<Boolean>("SilentSwitch", true));
    ...
    private EntityPlayer getTarget(double range, boolean trapped) {
        ...
        double distance = Math.pow(range, 2.0) + 1.0;
        for (EntityPlayer player : AutoWeb.mc.world.playerEntities) {
            if (EntityUtil.isntValid(player, range) || trapped && player.isInWeb || EntityUtil.getRoundedBlockPos(AutoWeb.mc.player).equals(EntityUtil.getRoundedBlockPos(player)) || OyVey.speedManager.getPlayerSpeed(player) > this.delay.getValue())
                continue;
            if (target == null) {
                target = player;
                distance = AutoWeb.mc.player.getDistanceSq(player);
